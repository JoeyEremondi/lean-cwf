...
namespace Subst
  ...
  theorem wkRenaming (ρ : Renaming m n)
    : ofRenaming  (ρ.wk) = (ofRenaming ρ).wk := by
    funext x
    simp [ofRenaming, Renaming.wk, wk, Renaming.shift, Renaming.rename]
    cases x <;> simp [Fin2.add]

  -- @[simp]
  theorem substOfRenaming {t : ABT n a} : {m : ℕ} →  {ρ : Renaming m n}
  ...
  theorem unOfRenaming {t : ABT n a} : {m : ℕ} →  {ρ : Renaming m n}
  ...
  @[simp]
  theorem sub_id (t : ABT n a) : t⦇id⦈ = t := by
    induction t
  ...
  -- @[simp]
  -- theorem sub_comp {t : ABT c tag} : ∀ {a} {b}, {θ : Subst a b} →  {θ2 : Subst b c} →
  ...
  @[local simp]
  theorem sub_rename_extL {ρ : Renaming b c} {θ : Subst a b} :
  ...
  @[local simp]
  theorem sub_rename_extR {ρ : Renaming a b} {θ : Subst b c} :
  ...
  @[local simp]
  theorem sub_rename_fusionL {t : ABT c tag} : ∀ {a} {b} {ρ : Renaming b c} {θ : Subst a b},
  ...
  @[local simp]
  theorem sub_rename_fusionR {t : ABT c tag} : ∀ {a} {b} {ρ : Renaming a b} {θ : Subst b c},
  ...
  @[simp]
  theorem ofRenameId : ofRenaming  (n := n) Renaming.id = id := by
    funext x
    simp [ofRenaming, Renaming.id, id]

  @[simp]
  theorem ofRenameIdDef : ofRenaming  (n := n) (fun x => x) = id := by
    funext x
    simp [ofRenaming, Renaming.id, id]
  ...
  @[simp]
  theorem comp_idL (θ : Subst m n) :
    θ ⨟ id = θ := by
    funext
    aesop

  @[simp]
  theorem comp_idR (θ : Subst m n) :
    id ⨟ θ = θ := by
    funext
    simp [comp]

  @[simp]
  theorem comp_assoc (θ1 : Subst a b) (θ2 : Subst b c) (θ3 : Subst c d)
    : θ1 ⨟ (θ2 ⨟ θ3) = (θ1 ⨟ θ2) ⨟ θ3  := by
    funext x
    simp [comp]

  @[simp]
  theorem sub_dist {t : Term c } {θ1 : Subst b c} {θ2 : Subst c a}
    : θ1⨟⟪θ2 ● t⟫ = ⟪θ1 ⨟ θ2 ● t⦇θ1⦈⟫ := by
      funext x
      cases x <;> simp [comp, ext]

  @[simp]
  theorem z_shift : ext (n := n)  proj x0 = id := by
    funext x
    simp [ext, proj, id]
    cases x <;> try aesop_cat

  @[simp]
  theorem sub_eta {θ : Subst m (Nat.succ n)} :
  ...
  @[simp]
  theorem sub_head {θ : Subst m n} {t : Term m} :
      (ext θ t) (Fin2.fz) = t := by simp [ext]

  @[simp]
  theorem sub_tail {θ : Subst m n} {t : Term m} :
      ext θ t ⨟ proj = θ := by
      funext x
      cases x <;> aesop_cat

  -- Useful when dealing with renamings
  theorem singleSubRename {s : Term (Nat.succ n)} {t : Term n} {ρ : Renaming m n}
    : s/[ t /x]⦇ρ⦈ᵣ = (s⦇Renaming.wk ρ⦈ᵣ)/[t⦇ρ⦈ᵣ /x] := by
      simp [wkRenaming, wk_def, substOfRenaming, Subst.subst, ABT.map]

  @[simp]
  theorem singleSubSub {s : Term (Nat.succ n)} {t : Term n} {θ : Subst m n}
...
@[simp]
  theorem renVecNil {θ : Renaming m n} : (depVecNil  (ss := ss))⦇θ⦈ᵣ = depVecNil := by
    simp [depVecNil]
    funext
    contradiction
...
-- def weaken (t : ABT n a) : ABT (n+1) a := by
